...
NSString * const GWNetworkDomain = API_HOST;
...
+ (instancetype)requestWithSuccessBlock:(GWRequestSuccessBlock)successBlock failureBlock:(GWRequestFailureBlock)failureBlock {
    return [[[self class] alloc] initWithSuccessBlock:successBlock failureBlock:failureBlock];
}

- (void)startCompletionBlockWithSuccess:(GWRequestSuccessBlock)success failure:(GWRequestFailureBlock)failure {
...
- (void)constructURL {
    _url = [NSString stringWithFormat:@"%@%@", GWNetworkDomain, [self requestURLPath]];
}
...
- (NSURLSessionTask *)sessionTaskForError:(NSError *__autoreleasing *)error {
    GWRequestMethod method = [self requestMethod];
...
- (AFHTTPRequestSerializer *)requestSerializer {
    ...
    switch ([self requestSerializerType]) {
        case GWRequestSerializerTypeHTTP: {
            requestSerializer = [AFHTTPRequestSerializer serializer];
        } break;
        case GWRequestSerializerTypeJSON: {
...
- (GWRequestMethod)requestMethod {
    return GWRequestMethodGET;
}

- (GWRequestSerializerType)requestSerializerType {
    return GWRequestSerializerTypeHTTP;
}
