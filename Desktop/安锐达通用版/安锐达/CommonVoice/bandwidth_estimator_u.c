...
#include "bandwidth_estimator_u.h"
#include "settings.h"
#include "isac.h"
...
int32_t WebRtcIsac_GetDownlinkBandwidth( const BwEstimatorstr1 *bwest_str)
{
  int32_t  rec_bw;
  float   jitter_sign;
  float   bw_adjust;
...
int32_t
WebRtcIsac_GetDownlinkMaxDelay(const BwEstimatorstr1 *bwest_str)
{
...
WebRtcIsac_GetUplinkBandwidth(
    const BwEstimatorstr1* bwest_str,
    int32_t*          bitRate)
{
  /* limit range of bottle neck rate */
...
int32_t
WebRtcIsac_GetUplinkMaxDelay(const BwEstimatorstr1 *bwest_str)
{
