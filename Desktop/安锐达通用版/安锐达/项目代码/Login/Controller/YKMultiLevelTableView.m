...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        self.contentView.backgroundColor = RGB(34, 34, 34, 1);
        ...
        _nodeLabel.font  =[UIFont systemFontOfSize:18];
        [self addSubview:_nodeLabel];
        
        _line = [[UIView alloc] initWithFrame:CGRectZero];
        _line.backgroundColor = [UIColor lightGrayColor];
        
        _rightImage = [[UIImageView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH-110-150-45, 15, 30, 30)];
...
- (void)moveNode:(CGFloat)indentationX{
    ...
    CGRect frame1 = CGRectMake(0, (cellHeight-18)/2, 18, 18);
    frame1.origin.x = indentationX;
    _leftImage.frame = frame1;
    
    CGRect frame2 = CGRectMake(0, cellHeight-1, cellWidth, PixelOne);
...
- (id)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        self.backgroundColor = RGB(34, 34, 34, 1);
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (cell.node.desModel) {
        if (onLineUsers.count > 0) {
            BOOL flag = YES;
            for (int i = 0; i < [onLineUsers allKeys].count; i ++) {
                if ([[onLineUsers allKeys][i] isEqualToString:[NSString stringWithFormat:@"%ld",cell.node.desModel.uId]]) {
                    if (![[onLineUsers allKeys][i] isEqualToString:[FunctionClass sharedInstance].uId]) {
                        cell.rightImage.image = [UIImage imageNamed:@"ic_hospital_online"];
                    } else {
                        cell.rightImage.image = [UIImage imageNamed:@"user"];
                    }
                    flag = NO;
                    break;
                }
            ...
            if (flag) {
                cell.rightImage.image = [UIImage imageNamed:@"ic_hospital_offline"];
            }
        }else {
            cell.rightImage.image = [UIImage imageNamed:@"ic_hospital_offline"];
        }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    YKNodeCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    if (currentNode.isLeaf) {
        if ([cell.rightImage.image isEqual:[UIImage imageNamed:@"ic_hospital_online"]]) {
...
- (void)setOnlines:(NSMutableDictionary *)onlines {
    _onlines = onlines;
    onLineUsers = onlines;
    [self reloadData];
}
